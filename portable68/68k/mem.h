...
u8 readByte(u32 addr, bool lastCycle = false) {
    ...
    moiracpu->sandbox.record(moira::PEEK8, addr & 0xffffff, cycleCounter, data);
    sync(2);
...
u16 readWord(u32 addr, bool lastCycle = false) {
    ...
    moiracpu->sandbox.record(moira::PEEK16, addr & 0xffffff, cycleCounter, word);
    sync(2);
    // printf("portable68000:: %d\n", cycleCounter);
	return word;
}
...
void writeByte(u32 addr, u8 value, bool lastCycle = false) {
    ...
    moiracpu->sandbox.record(moira::POKE8, addr & 0xffffff, cycleCounter, value);
    sync(2);
}
...
void writeWord(u32 addr, u16 value, bool lastCycle = false) {
    ...
    moiracpu->sandbox.record(moira::POKE16, addr & 0xffffff, cycleCounter, value);
    sync(2);
}
